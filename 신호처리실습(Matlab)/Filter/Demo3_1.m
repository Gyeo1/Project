...
%설명 : 그림 순서대로 버터워스->체비셰브 타입1->체비셰브 타입2->일렙픽 필터이다.
%figure1에서는 비교할만한 점은 버터워스와 체비셰브타입1은 pole의 해만 있고 zero가없고
%나머지 2필터는 pole과 zero의 해가 둘다 있다.그리고 모든 필터는 안정한 상태가 되야되므로
%pole들이 좌반평면상에 존재합니다.
%figure2에서 알 수있는 점은 버터워스는 1rad/sec에서 천이대역이 시작되며
