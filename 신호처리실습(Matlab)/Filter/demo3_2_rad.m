...
H1=freqs(num,den,w*2*pi); %num:전달함수 H(s)의 분자 다항식계수
                    %den: H(s)의 분모 다항식 계수벡터 /w=관찰하고자하는 주파수
H_1=10*log10((abs(H1)).^2); %주파수 응답의 크기 그래프.

%2 앞선 필터를 다른 필터로 변경
%numt=변환된 LPF의 분자 다항식 ,dent는 분모 다항식
[numt1,dent1]=lp2lp(num,den,300*2*pi); %300은 cutoff주파수 hz인데 rad/s변환!!!(핵심)
H=freqs(numt1,dent1,Hz*2*pi);
H_2=10*log10((abs(H)).^2);
%lp2bp는 (전달함수 분자,전달함수 분모,중심주파수,대역폭이다(
[numt2,dent2]=lp2bp(num,den,650*2*pi,700*2*pi);%passband가 300~1000Hz 중싱은 650, 길이는 700
H=freqs(numt2,dent2,Hz*2*pi);
H_3=10*log10((abs(H)).^2);

[numt3,dent3]=lp2bp(num,den,1500*2*pi,1000*2*pi);%passband가 1000~2000Hz 중싱은 1500, 길이는 1000
H=freqs(numt3,dent3,Hz*2*pi);
H_4=10*log10((abs(H)).^2);

[numt4,dent4]=lp2bp(num,den,3000*2*pi,2000*2*pi);
H=freqs(numt4,dent4,Hz*2*pi);
H_5=10*log10((abs(H)).^2);

[numt3,dent3]=lp2hp(num,den,4000*2*pi);
H=freqs(numt3,dent3,Hz*2*pi);
