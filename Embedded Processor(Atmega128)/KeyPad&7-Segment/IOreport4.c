...
while(1)
{
    ...
    if(New_key_data)     //키가 눌릴때.
        {   
         count++;//새로운 키값이 들어오면 카운터증가.
         if(New_key_data%4 !=0)            //모드키가 아닐때
          {
             key_Num =(New_key_data/4)*3+(New_key_data%4);  //키값을 재정비
            
             if(key_Num>=10 && key_Num==11)      //10넘으면 * 0 #이므로 따로 구분 숫자만
             {
               key_Num=0;
               }                                     
              else;
                
              if (count>4)//segment를 4개 다쓰면 카운터가 4초과.
                { 
                  //segment를 0000으로 초기화하고 count도 0으로 초기화..
                  count=0; 
                  buf_seg[3]=0;
                  buf_seg[2]=0;
                  buf_seg[1]=0;
          ...
          else//키가 때졌을때 (= ney key data가 0일때)
              Key_off_flag=1;    
